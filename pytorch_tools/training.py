...
class Trainer:
    ...
    def __init__(
        ...
        if val_loss is None:
            self.val_loss = dict(loss_func=loss_func, channels=np.s_[:])
        else:
        ...
        if writer_path is not None:
            if not os.path.exists(writer_path):
                os.mkdir(writer_path)
            self.writer_train = SummaryWriter(os.path.join(writer_path, 'train'))
            self.writer_val = SummaryWriter(os.path.join(writer_path, 'val'))
        else:
        ...
        if self.optimizer is None:
            # optimizer = t.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),
            #                          lr=1e-4, betas=(0.9, 0.999), weight_decay=l2_reg_param)
            self.optimizer = t.optim.Adam(model.parameters(),
    ...
    def _on_epoch_end(self, train_loss, epoch):
        print('------------------------------------------')
        print('Epoch ended, evaluating model ...')
        ...
        with t.no_grad():
            ...
            while eval_not_done:
                ...
                valx, valy, val_epoch, valn, loe = next(self.val_generator)

                # Forward pass
                tensx = t.tensor(np.moveaxis(valx, 4, 1), dtype=t.float32).cuda()
                predy = self.model(tensx)

                # Compute loss
                tensy = t.tensor(np.moveaxis(valy, 4, 1), dtype=t.float32).cuda()
                loss = self.val_loss['loss_func'](predy[self.val_loss['channels']], tensy)
                sum_loss += loss
            ...
            if self.writer_val:
                self.writer_val.add_scalar('losses/loss', val_loss, epoch)
    ...
    def train(self):
        ...
        for it_idx, (x, y, epoch, n, loe) in enumerate(self.train_generator):
            ...
            # Compute loss
            tensy = t.tensor(np.moveaxis(y, 4, 1), dtype=t.float32).cuda()
            loss = self.loss_func(predy, tensy)
            ...
            if loe:
                ...
                if self.writer_train:
                    self.writer_train.add_scalar('losses/loss', train_loss, epoch)

                # Evaluate previous epoch
                improvement = self._on_epoch_end(train_loss, epoch)

                # Callbacks when the model improved
                if self.callbacks is not None and improvement:
                    with t.no_grad():
                        self.model.eval()
                        print('Validation loss improved! Computing callbacks ...')
                        with t.no_grad():
